...
void setup() {
  ...
  if ( OTAmode() ) {
    ...
    if (rtcMem.crc32  == crc32Calc ) {    
      ...
      maintainClock();
      setSensorType();
      readSensors(true);   //and save data to LittleFS or transmit
      writeToRTCMemory();   //after maybe changing fNum
      delay(2);  
      ESP.deepSleep( 1e3*(60e3*rtcMem.rtcData.timings[0] + 600 - millis() ), WAKE_RF_DISABLED); // fine adjustment
    } else { 
      //>>>>>>>>>>>>>serve web page<<<<<<<<<<<<<  
...
String readSensors(bool bSaveData) {   //and save to file/transmit 
...
void showSetup() {
  ...
  server.send(200, "text/html", "<h1>SETUP</h1>"
      ...
      "<br><br>"
      " This should show reasonable values for temperature,humidity: " + readSensors(false) +  " <br><br>"
      " Enter the location &nbsp; &nbsp; "       
...
void transmitData(int iTemp, int iHumid ) {
  unsigned long ulData; //value to return
  long lTempP50_100;
  delay(10);
  mySwitch.setProtocol(6);
  mySwitch.setPulseLength(200); 
  mySwitch.enableTransmit(transmitPIN);   //using pin 12
...
void shareData() {
 ...
 // server.sendContent(sHead ); 
  ...
  yield();
  WiFi.mode(WIFI_OFF);
  unsigned long napTime = 60e3*rtcMem.rtcData.timings[0] - ulAdjTime;
  if (napTime > 30*60e3 ) napTime = 1000;  //max sleep should be interval less time to here (1 minute + 4 minutes). Play safe use 30 minutes
...
void saveRecord(String output)  {
  ...
  Serial.print(" file size : ");Serial.println(f2.size());
  f2.println(output);
  delay(10);
  f2.close();
...
//only at startup or after delete all files
void checkFileState() {  
  ...
  if (!LittleFS.exists("/SensorMetaFile.txt" ) ) {
    ...
    Serial.print(" checkfile state filenum ");Serial.println(sFileNum);
    f1.close();
...
void showExtraInputs() {   //process
  ...
  displayparas( 2, "rtcMem.rtcData.timings[0]   ", String(rtcMem.rtcData.timings[0]));
  displayparas( 2, "rtcMem.rtcData.iSave2  "  , String(rtcMem.rtcData.iSave2) );
...
void displayparas(int where, String descr, String val) {
  Serial.print(where);Serial.print("  ");Serial.print(descr); Serial.print("  "); Serial.println(val);
}
